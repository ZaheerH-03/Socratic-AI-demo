...
// Socratic AI system prompt template
const getSystemPrompt = (topic: string, level: string, turnCount: number, trapSprung: boolean) => {
...
app.post("/api/chat", async (req, res) => {
  try {
    ...
    // Exclude the initial system prompt from the client history if any
    ...
    const response = await ai.models.generateContent({
      ...
      config: {
        systemInstruction: getSystemPrompt(topic, level, currentTurn, trapSprung),
        temperature: 0.8,
...
app.post("/api/report", async (req, res) => {
  try {
    ...
    const reportPrompt = `You are the Examination Board reviewing the student's performance on the oral exam.
Subject: "${topic}"
